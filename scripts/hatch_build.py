...
binary_to_tag = {
    ...
    "deno-aarch64-unknown-linux-gnu.zip": f"py3-none-manylinux_{'_'.join(MIN_SUPPORTED_GLIBC)}_aarch64",
    "deno-x86_64-pc-windows-msvc.zip": "py3-none-win_amd64",
    "deno-x86_64-unknown-linux-gnu.zip": f"py3-none-manylinux_{'_'.join(MIN_SUPPORTED_GLIBC)}_x86_64",
}
...
def detect_platform() -> tuple[str, str]:
    ...
    if system == "linux":
        ...
        if version_tuple < MIN_SUPPORTED_GLIBC:
            raise RuntimeError(f"The minimum supported version of glibc is {'.'.join(MIN_SUPPORTED_GLIBC)}")
